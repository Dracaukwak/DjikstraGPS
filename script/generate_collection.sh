...
while getopts "r:n:d:i:h" option; do
  case $option in
  r)
    dir=$OPTARG;;
  n)
    IFS=':' read nb_vertices_min nb_vertices_max nb_vertices_step <<< $OPTARG;;
  d)
    IFS=':' read d_min d_max d_step <<< $OPTARG;;
  i)
    nb_instances=$OPTARG;;
  h)
    usage;;
...
echo "DOSSIER : "$dir
echo "Sommets : " $nb_vertices_min $nb_vertices_max $nb_vertices_step
echo "Densité : " $d_min $d_max $d_step
echo "Instance : " $nb_instances
...
# Création du dossier dir s'il n'existe pas déjà

mkdir -p $dir


for sommet in $(seq $nb_vertices_min $nb_vertices_step $nb_vertices_max ); do
  for density in $(seq $d_min $d_step $d_max); do
    for instance in $(seq 1 $nb_instances); do

      nomFichier="${sommet}_${density}_${instance}"
      ./generate_instance.sh "$dir/$nomFichier" "$sommet" "$density"
